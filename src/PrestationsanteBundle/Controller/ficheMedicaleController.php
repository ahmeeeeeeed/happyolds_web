...
class ficheMedicaleController extends Controller
{
   ...
   //var_dump($tabIdFiche);
    ...
    public function generateFichesPDFAction($idResident)
    {
        $em=$this->getDoctrine()->getManager();
        $fiches=$em->getRepository("PrestationsanteBundle:PrestationSante")->findBy(array('idResident'=>$idResident));
        $resident=$em->getRepository("MaisonretraiteBundle:Resident")->find($idResident);
        ...
        $html = $this->renderView('@Prestationsante/dossierMedicale/fichesPDF.html.twig',array(
            'f'=>$tabFiche,
            'resident'=>$resident));

        $filename = 'Fiches de '.$resident->getNomResident();
