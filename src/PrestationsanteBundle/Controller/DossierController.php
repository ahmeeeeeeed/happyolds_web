...
//var_dump($idDossier);
    ...
    public function generateDossierPDFAction($idResident){
        $em=$this->getDoctrine()->getManager();
        $dossier=$em->getRepository("PrestationsanteBundle:DossierMedicale")->findOneBy(array('idResident'=>$idResident));
        ...
        $html = $this->renderView('@Prestationsante/dossierMedicale/dossierPDF.html.twig',array(
            'dossier'=> $dossier,
            'resident'=> $resident,
            'allergies'=>$allergie,
            'prescriptions'=>$prescription
        ...
        $filename = 'Dossier de '.$resident->getNomResident();
    ...
    /*    return  $this->get('knp_snappy.pdf')->generateFromHtml(
            $this->renderView(
                '@Prestationsante/dossierMedicale/dossierPDF.html.twig'
            ),
            'C:\wamp64\www\PidevHappyoldsSymfony\web\dossiers\fileeeee.pdf'
        );*/
